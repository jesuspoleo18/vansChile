    ...
    $.fn.ADMAKEadvancedFilter = function (params) {
        ...
        var methods = {
            init: function () {
                if (methods.verificaElemento(elements.$menu)) {
                    ...
                    if ($('.search-single-navigator').length) {
                        elements.$subTitulos = $('.search-single-navigator ' + options.elSubTitulo,
                            options.menu)
                    }
            ...
            criaEstrutura: function () {
                elements.$menu.after(
                    '<div class="admake-advanced-filter" style="display: none;">' +
            ...
            criaBoxes: function () {
                $.each(elements.$subTitulos, function () {
                    ...
                    if (arrayTipos[subTituloAux] !== undefined && arrayTipos[
                        subTituloAux] !== null && arrayTipos[subTituloAux] !== '') {
                    ...
                    $estrutura.append(
                        options.estruturaBox
                            ...
                            .replace(/\{{class}}/g, classAux + ' filtro-' +
                                subTituloAux)
            ...
            preparaBoxImage: function () {
                $.each($('.image', $estrutura), function () {
                    ...
                    $.each($itens, function () {
                        var $t = $(this);
                        var titulo = methods.removeCaracteres(($t.attr(
                            'title') !== undefined && $t.attr(
                                'title') !== null && $t.attr('title') !==
                            '') ? $t.attr('title') : $t.html());
                        if ($this.attr('data-type') !== undefined && $this.attr(
                            'data-type') !== null && $this.attr(
                                'data-type') !== '') {
                            var imagem = $this.attr('data-type') + '-' +
                                titulo + '.' + options.extensaoImagens;
                        } else {
                            var imagem = titulo + '.' + options.extensaoImagens;
                        }
                        $t.html('<img src="/arquivos/' + imagem.trim() +
                            '" title="' + $t.html() + '" alt="' + $t.attr(
                                'title') + '" class="filtro-img" >');
                    });
                });
            },
            preparaBoxRange: function () {
                if (!$.fn.ionRangeSlider) {
                    console.log(
                        'Para habilitar a op̤̣o RANGE ̩ necessario inserir o plugin Range.Slider - http://www.jqueryrain.com/'
                    );
                    return false;
                }
                var aux = 0;
                ...
                $.each($('.range', $estrutura), function () {
                    ...
                    var $itens = $('li a', $this);
                    $lista.hide();
                    id = ($this.attr('data-type') !== undefined && $this.attr(
                        'data-type') !== null && $this.attr('data-type') !==
                        '') ? 'range-' + $this.attr('data-type') : 'range-' + aux++;
                    $opcoes.append('<div id="' + id + '"></div>');
                    var values = [];
                    $.each($itens, function () {
                        var $t = $(this);
                        var titulo = $t.attr('title');
                        $t.attr('id', 'option-' + methods.removeCaracteres(
                            titulo));
                        values.push(titulo);
                    });
                    $("#" + id).ionRangeSlider({
                        ...
                        onFinish: function (data) {
                            var aux = $('#option-' + methods.removeCaracteres(
                                data.from_value));
                            if (aux.length) {
